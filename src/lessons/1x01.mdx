...
import Hint from "~/app/components/helpers/hint.tsx"
...
This is a diagram of what the inside of a transistor <Hint>This particular transistor is an enhancement-type N-channel MOSFET.</Hint> might look like. The yellow regions have a small negative electric charge and the blue region has a small positive electric charge. <Hint>Ordinarily, silicon is electrically neutral, but for use in transistors, it is altered through special processes called "N-type doping" (for the yellow regions) and "P-type doping" (for the blue region).</Hint>

You might remember from one of your science classes that negative charge occurs when there are extra electrons and positive charge occurs when there aren't enough, so let's add some electrons and empty spaces where electrons want to go to our diagram.